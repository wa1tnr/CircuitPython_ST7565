...
"""
    mbytes = bytearray(b'\xfc\x02\x1c\x02\xfc\x00') # W - deshipu driver leads this idea
    char_out()
    mbytes = bytearray(b'\x3e\x48\x88\x48\x3e\x00') # A
    char_out()
    mbytes = bytearray(b'\x00\x42\xfe\x02\x00\x00') # 1
    char_out()
    mbytes = bytearray(b'\xc0\x80\xfe\x80\xc0\x00') # T
    char_out()
    mbytes = bytearray(b'\xfe\x20\x10\x08\xfe\x00') # N
    char_out()
    mbytes = bytearray(b'\xfe\x90\x98\x94\x62\x00') # R
    char_out()
    mbytes = bytearray(b'\x10\x10\x10\x10\x10\x00') # dash
    char_out()
    mbytes = bytearray(b'\x82\x84\x88\x90\xe0\x00') # 7
    char_out()
"""
...
mbytes = bytearray(b'\x10\x03\xb2\xe0') # 7
for p in range(0,4):
    cmd = mbytes[p]
